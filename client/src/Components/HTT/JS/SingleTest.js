...
import {faNewspaper,faFilePdf,fa,} from "@fortawesome/free-regular-svg-icons";
...
const FullUrl = () => {
  ...
  const [selectedEdition, setSelectedEdition] = useState( selectedRawEdition || "Chennai");
  ...
  const handleProductChange = (event) => {
    ...
    axiosInstance.get(`/htt/getEditionFilenames?date=${formattedDate}&product=${selectedProduct}`)
      .then((response) => {
  ...
  const handleDownloadClick = async () => {
    ...
    try {
      ...
      const uniqueFileName = `${selectedDate}_${selectedEdition}_${new Date().getTime()}.pdf`;
